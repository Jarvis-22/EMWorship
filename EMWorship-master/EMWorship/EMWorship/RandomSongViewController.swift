...
class RandomSongViewController: UIViewController {
    ...
    func randomize (){
        ...
        s1 = Int(arc4random_uniform(UInt32(temp.count)))
        randomArray.append(s1)
        s2 = Int(arc4random_uniform(UInt32(temp.count)))
        while s2 == s1
        {
             s2 = Int(arc4random_uniform(UInt32(temp.count)-1))
        }
       randomArray.append(s2)
        s3 = Int(arc4random_uniform(UInt32(temp.count)-1))
        while s3 == s2 || s3 == s1
        {
            s3 = Int(arc4random_uniform(UInt32(temp.count)-1))
        }
